...
jQuery.noConflict();
(function effectsClassCreator($){
	var effectsTag=[];
	var effects;
		$("document").ready(function(){
			$(".views-row").each(function(index){
				$(this).attr("id","parallax_container"+index);
			});

			$(".views-field-field-new-effects .field-content").each(function(index){
				effects= $(".views-field-field-new-effects .field-content").toArray();
				effectsTag.push(effects[index].innerHTML);

				if (index < effectsTag.length) {
        			$(this).closest("[id*='parallax_container']").addClass(effectsTag[index].toLowerCase().replace(/ /g, ''));
    			}
